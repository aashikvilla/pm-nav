...
export default function EducationForm({ entity, onClose }: Props) {
  const router = useRouter();
  const isEdit = !!entity;

  const [school, setSchool] = useState(entity?.school ?? "");
  const [degree, setDegree] = useState(entity?.degree ?? "");
  const [fieldOfStudy, setFieldOfStudy] = useState(entity?.fieldOfStudy ?? "");
  const [startDate, setStartDate] = useState(toMonth(entity?.startDate));
  const [endDate, setEndDate] = useState(toMonth(entity?.endDate));
  const [gpa, setGpa] = useState(entity?.gpa ?? "");
